...
async def get_binary_evaluation(
    ...
    base_prompt = f"""
Ты — эксперт-оценщик студенческих работ. Твоя задача — оценить работу ЧЁТКО, но СПРАВЕДЛИВО, с учётом, что это учебная работа, а не научная статья.
...
1. ЗАЧТЕНО — если работа:
   - содержит хотя бы краткое, но конкретное описание темы исследования;
   - указывает направление применения ИИ (пусть и в общих чертах, но по смыслу связано с темой);
   - содержит хотя бы минимальное обоснование выбора темы (даже простое личное или практическое);
   - имеет упоминание ожидаемых результатов (пусть краткое или неформальное);
   - содержит не менее ~60–70 слов СОДЕРЖАТЕЛЬНОГО текста, даже если есть стилистические ошибки;
   - не выглядит полностью сгенерированной ИИ и показывает попытку самостоятельной работы.

2. НЕ ЗАЧТЕНО — если работа:
   - состоит почти полностью из шаблонных или бессодержательных фраз;
   - не имеет конкретной темы, направления или аргументации;
   - слишком короткая (менее 50 слов);
   - полностью выглядит как машинный шаблон без признаков человеческого участия.

💬 ВАЖНО:
- Оценивай по содержанию, а не по стилю. Если студент выражает мысли неловко, но по сути затрагивает тему — засчитай.
- Если есть сомнение между “зачтено” и “не зачтено”, выбери “зачтено с замечаниями”.
- Работы без признаков ИИ-генерации должны получать преимущество (лояльность к живому стилю и ошибкам).

Верни результат в формате JSON:
{
  "result": "зачтено" или "не зачтено",
  "comment": "подробная оценка: что сделано хорошо, что требует улучшения, рекомендации студенту"
}
